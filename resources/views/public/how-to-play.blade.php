<x-public-layout title="Cara Bermain - {{ config('app.name') }}">
    ...
    <section class="relative overflow-hidden bg-gradient-to-b from-[#EAF3FF] via-[#F4F8FF] to-white dark:from-slate-900 dark:via-slate-900 dark:to-slate-900">
        {{-- Decorative light --}}
        <div aria-hidden="true" class="pointer-events-none absolute left-1/4 top-0 h-64 w-64 -translate-x-1/2 -translate-y-1/3 rounded-full bg-white/70 blur-3xl dark:bg-primary-500/10"></div>
        <div aria-hidden="true" class="pointer-events-none absolute right-1/4 bottom-0 h-72 w-72 translate-x-1/3 translate-y-1/3 rounded-full bg-blue-200/40 blur-3xl dark:bg-blue-500/10"></div>
        <div aria-hidden="true" class="pointer-events-none absolute left-1/2 top-1/2 h-[420px] w-[420px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[radial-gradient(circle,rgba(255,255,255,.7)_0%,rgba(255,255,255,0)_70%)] blur-2xl dark:opacity-20"></div>

        {{-- Soft fade into the section below --}}
        <div aria-hidden="true" class="pointer-events-none absolute inset-x-0 bottom-0 h-40 bg-[linear-gradient(to_bottom,rgba(255,255,255,0),rgba(255,255,255,.35),rgba(255,255,255,.75),rgba(255,255,255,1))] sm:h-48 lg:h-56 dark:bg-[linear-gradient(to_bottom,rgba(15,23,42,0),rgba(15,23,42,.35),rgba(15,23,42,.75),rgba(15,23,42,1))]"></div>
        ...
        <div class="relative mx-auto flex max-w-7xl flex-col items-center gap-6 px-4 py-16 sm:px-6 md:flex-row md:justify-between md:gap-6 lg:gap-10 lg:px-8 lg:py-20">
            ...
            <div class="text-center md:flex-1">
                <h1 class="text-4xl font-extrabold tracking-tight text-slate-900 sm:text-5xl dark:text-slate-100">Cara Bermain</h1>
    ...
    <section class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 lg:gap-x-10 lg:gap-y-10">
            @foreach ($steps as $step)
                <div class="group relative flex flex-col items-center rounded-2xl bg-white p-6 text-center shadow-sm transition-all duration-200 hover:-translate-y-1 hover:shadow-soft dark:bg-slate-800 dark:shadow-none">
                    ...
                    <h3 class="mt-4 font-bold text-slate-900 dark:text-slate-100">{{ $step['judul'] }}</h3>
                    <p class="mt-1.5 text-sm text-slate-600 dark:text-slate-400">{{ $step['deskripsi'] }}</p>
                    ...
                    @if ($loop->iteration % 5 !== 0 && ! $loop->last)
                        <span class="pointer-events-none absolute right-0 top-1/2 hidden -translate-y-1/2 translate-x-1/2 lg:flex" aria-hidden="true">
                            <svg viewBox="0 0 40 16" class="h-4 w-9 text-primary-300 dark:text-primary-500/40" fill="none">
    ...
    <section class="mx-auto max-w-7xl px-4 pb-20 sm:px-6 lg:px-8">
        <div class="relative flex flex-col items-center gap-5 overflow-hidden rounded-3xl bg-gradient-to-r from-primary-50 via-blue-50 to-primary-50 p-6 text-center shadow-sm sm:gap-6 sm:p-8 md:flex-row md:justify-between md:gap-6 md:text-left lg:gap-10 dark:from-primary-500/10 dark:via-blue-500/10 dark:to-primary-500/10">
            {{-- Decorative light --}}
            <div aria-hidden="true" class="pointer-events-none absolute left-8 top-1/2 h-32 w-32 -translate-y-1/2 rounded-full bg-white/70 blur-3xl dark:bg-primary-500/10"></div>
            ...
            <div class="relative md:flex-1">
                <h2 class="text-xl font-bold text-slate-900 sm:text-2xl dark:text-slate-100">Siap Menguji Pengetahuanmu?</h2>
                <p class="mt-1 text-slate-600 dark:text-slate-400">Belajar jadi lebih seru, statistik jadi lebih mudah dipahami!</p>
            </div>
