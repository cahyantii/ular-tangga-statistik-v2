...
<x-player-layout>
    <x-slot name="header">
        <h1 class="text-xl font-bold text-slate-800 sm:text-2xl dark:text-dark-text">Leaderboard</h1>
        <p class="mt-0.5 text-sm text-slate-500 dark:text-dark-muted">Bersaing dan raih posisi terbaikmu! &#127942;</p>
    </x-slot>
    ...
    <div class="space-y-6">
        <div class="flex flex-wrap gap-2">
            @foreach ($tabs as $key => $item)
                ...
                <a
                    ...
                    class="inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold shadow-sm transition-all duration-200 {{ $active ? 'bg-gradient-to-r from-primary-500 to-primary-600 text-white shadow-soft dark:from-primary-600 dark:to-primary-700' : 'bg-white text-slate-600 hover:-translate-y-0.5 hover:bg-slate-50 dark:bg-dark-surface dark:text-dark-text dark:hover:bg-dark-surface2' }}"
                >
                    @if ($item['icon'])
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div class="space-y-6 lg:col-span-2">
                ...
                <div class="overflow-hidden rounded-3xl bg-white shadow-sm transition-shadow duration-300 hover:shadow-soft dark:bg-dark-surface dark:shadow-none">
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-sm">
                            <thead class="bg-slate-50 text-xs uppercase tracking-wide text-slate-500 dark:bg-slate-800/50 dark:text-dark-muted">
                                <tr>
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50">
                                @if ($board['table']->isEmpty() && ! $board['current'])
                                    <tr>
                                        <td colspan="6" class="px-4 py-8 text-center text-slate-400 sm:px-6 dark:text-dark-muted">
