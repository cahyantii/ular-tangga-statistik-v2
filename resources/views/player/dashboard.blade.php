<x-player-layout>
    <div class="space-y-6">
        ...
        <div class="relative min-h-[170px] overflow-hidden rounded-[28px] bg-white p-6 shadow-sm sm:p-8 dark:bg-dark-surface dark:shadow-none">
            {{-- Background image layer for light mode --}}
            <div
            ...
            <div
                ...
                style="background-image: linear-gradient(90deg, rgba(30,41,59,0.97) 0%, rgba(30,41,59,0.92) 25%, rgba(30,41,59,0.55) 50%, rgba(30,41,59,0) 72%), url('{{ asset('images/brand/logo-dhas.png') }}'); background-size: cover; background-position: center; background-repeat: no-repeat;"
            ></div>
            <div class="relative z-10 flex h-full max-w-md flex-col justify-center">
                <h1 class="text-2xl font-bold text-slate-800 sm:text-3xl lg:text-[42px] dark:text-dark-text">
                    Halo, {{ auth()->user()->name }} <span class="inline-block animate-float">&#128075;</span>
                </h1>
                <p class="mt-2 text-lg text-slate-600 dark:text-dark-muted">Siap bermain dan belajar hari ini?</p>
            </div>
        </div>
        ...
        @if ($activeSession)
            ...
            @endphp
            <a href="{{ $continueUrl }}"
               class="block rounded-2xl border-2 border-secondary-400 bg-secondary-50 p-6 transition-all duration-300 hover:-translate-y-0.5 hover:bg-secondary-100 hover:shadow-soft dark:border-secondary-600 dark:bg-secondary-900/30 dark:hover:bg-secondary-900/50">
                <p class="text-sm font-medium text-secondary-600 dark:text-secondary-400">Sesi permainan sedang berlangsung</p>
                <p class="mt-1 text-xl font-bold text-secondary-700 dark:text-secondary-300">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            {{-- Progress belajar --}}
            <div class="rounded-3xl bg-white p-5 shadow-sm transition-shadow duration-300 hover:shadow-soft sm:p-6 lg:col-span-2 dark:bg-dark-surface dark:shadow-none">
                <div class="mb-5 flex items-center justify-between">
                    <h3 class="flex items-center gap-2 text-base font-bold text-slate-700 dark:text-dark-text">
                        <x-player.icon name="chart-bar" class="h-5 w-5 text-primary-500 dark:text-primary-400" />
            ...
            {{-- Achievement teaser --}}
            <div class="rounded-3xl bg-white p-5 shadow-sm transition-shadow duration-300 hover:shadow-soft sm:p-6 dark:bg-dark-surface dark:shadow-none">
                <div class="mb-5 flex items-center justify-between">
                    <h3 class="flex items-center gap-2 text-base font-bold text-slate-700 dark:text-dark-text">
                        <x-player.icon name="trophy" class="h-5 w-5 text-accent-500 dark:text-accent-400" />
                        Achievement
                    </h3>
                ...
                <p class="mb-4 text-sm text-slate-500 dark:text-dark-muted">{{ $totalAchievementDiraih }} achievement telah diraih.</p>

                @if ($nearestAchievement)
                    <div class="rounded-2xl bg-accent-50 p-4 dark:bg-accent-900/20">
                        ...
                        <div class="mt-2.5 h-2.5 w-full rounded-full bg-accent-100 dark:bg-accent-900/50">
                ...
                @else
                    <p class="text-sm text-slate-400 dark:text-dark-muted">Semua achievement aktif telah diraih. Selamat!</p>
                @endif
            </div>
        </div>
