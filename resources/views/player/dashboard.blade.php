    ...
    <div class="space-y-6">
        ...
        <div
            ...
            x-bind:style="$store.playerTheme.isDark
                ? 'background-image: linear-gradient(90deg, rgba(15,23,42,0.97) 0%, rgba(15,23,42,0.92) 25%, rgba(15,23,42,0.55) 50%, rgba(15,23,42,0) 72%), url(\'{{ asset('images/brand/logo-dhas.png') }}\'); background-size: cover; background-position: center; background-repeat: no-repeat;'
                : 'background-image: linear-gradient(90deg, rgba(255,255,255,0.97) 0%, rgba(255,255,255,0.92) 25%, rgba(255,255,255,0.55) 50%, rgba(255,255,255,0) 72%), url(\'{{ asset('images/brand/logo-dhas.png') }}\'); background-size: cover; background-position: center; background-repeat: no-repeat;'"
        >
            <div class="relative z-10 flex h-full max-w-md flex-col justify-center">
                <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-100 sm:text-3xl lg:text-[42px]">
                    Halo, {{ auth()->user()->name }} <span class="inline-block animate-float">&#128075;</span>
                </h1>
                <p class="mt-2 text-lg text-slate-600 dark:text-slate-300">Siap bermain dan belajar hari ini?</p>
            </div>
        </div>
        ...
        @if ($activeSession)
            ...
            @endphp
            <a href="{{ $continueUrl }}"
               class="block rounded-2xl border-2 border-secondary-400 bg-secondary-50 p-6 transition-all duration-300 hover:-translate-y-0.5 hover:bg-secondary-100 hover:shadow-soft dark:border-secondary-500/40 dark:bg-secondary-950/40 dark:hover:bg-secondary-900/60 dark:hover:shadow-none">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div class="rounded-3xl bg-white p-5 shadow-sm transition-shadow duration-300 hover:shadow-soft dark:bg-slate-800 sm:p-6">
                ...
                @if ($nearestAchievement)
                    <div class="rounded-2xl bg-accent-50 p-4 dark:bg-amber-950/40 dark:border dark:border-amber-900/40">
                        <p class="text-sm font-semibold text-accent-600 dark:text-amber-400">{{ $nearestAchievement['achievement']->nama }}</p>
                        <p class="mt-1 text-xs text-accent-600/80 dark:text-amber-400/80">
                            {{ $nearestAchievement['current'] }} / {{ $nearestAchievement['target'] }}
                            ({{ $nearestAchievement['achievement']->syarat_type->label() }})
                        </p>
                        <div class="mt-2.5 h-2.5 w-full rounded-full bg-accent-100 dark:bg-slate-700">
