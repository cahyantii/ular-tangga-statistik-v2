<x-admin-layout>
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 sm:text-3xl">Feedback &amp; Laporan Bug</h1>
            <p class="mt-1 text-slate-500">Masukan dan laporan bug yang dikirim pemain</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)]">
            ...
            <p class="mt-4 text-sm font-medium text-slate-600">Total Masukan</p>
        ...
        <div class="rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)]">
            ...
            <p class="mt-4 text-sm font-medium text-slate-600">Baru</p>
            <p class="mt-1 text-3xl font-extrabold text-slate-900">{{ number_format($stats['baru']) }}</p>
        </div>
        <div class="rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)]">
            ...
            <p class="mt-4 text-sm font-medium text-slate-600">Dibaca</p>
            <p class="mt-1 text-3xl font-extrabold text-slate-900">{{ number_format($stats['dibaca']) }}</p>
        </div>
        <div class="rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)]">
            ...
            <p class="mt-4 text-sm font-medium text-slate-600">Selesai</p>
            <p class="mt-1 text-3xl font-extrabold text-slate-900">{{ number_format($stats['selesai']) }}</p>
        </div>
    </div>
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div class="min-w-0">
                        <div class="flex flex-wrap items-center gap-2">
                            <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold {{ $item->type->value === 'bug' ? 'bg-red-50 text-red-600' : 'bg-blue-50 text-blue-600' }}">
                        ...
                        <p class="mt-2 font-semibold text-slate-800">{{ $item->subject }}</p>
                        <p class="mt-1 text-sm text-slate-500">{{ $item->message }}</p>
                        <p class="mt-2 text-xs text-slate-400">Dari: {{ $item->user->name ?? 'Pengguna dihapus' }} ({{ $item->user->email ?? '-' }})</p>
                    </div>
                    <div class="flex shrink-0 items-center gap-2">
                        <form method="POST" action="{{ route('admin.management.feedback.update-status', $item) }}">
                            ...
                            <select name="status" onchange="this.form.submit()"
                                    class="rounded-xl border-slate-200 text-sm text-slate-600 shadow-sm focus:border-green-500 focus:ring-green-500">
                        ...
                        <form method="POST" action="{{ route('admin.management.feedback.destroy', $item) }}"
                            ...
                            <button type="submit" aria-label="Hapus"
                                    class="flex h-9 w-9 items-center justify-center rounded-lg border border-red-200 text-red-500 transition duration-200 ease-in-out hover:border-red-400 hover:bg-red-50">
