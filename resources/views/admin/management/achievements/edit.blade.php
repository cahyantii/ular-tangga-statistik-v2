<x-admin-layout>
    ...
    <form method="POST" action="{{ route('admin.management.achievements.update', $achievement) }}" class="max-w-xl space-y-4 rounded-2xl border border-slate-200 bg-white p-6">
        ...
        <x-admin.input label="Icon (opsional)" name="icon" :value="$achievement->icon" />
        <x-admin.input label="Warna Badge (opsional, kode hex)" name="warna_badge" :value="$achievement->warna_badge" />

        <x-admin.select label="Syarat" name="syarat_type" required
                         :options="['total_menang' => 'Total Kemenangan', 'akurasi_keseluruhan' => 'Akurasi Keseluruhan (%)', 'total_permainan' => 'Total Permainan']"
