<x-admin-layout>
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-4xl font-bold tracking-tight text-slate-900 dark:text-slate-100 sm:text-5xl">Kelola Materi</h1>
            <p class="mt-2 text-slate-500 dark:text-slate-400">Kelola semua materi pembelajaran yang tersedia dalam sistem</p>
        </div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)] transition duration-300 ease-in-out hover:-translate-y-[3px] hover:shadow-[0_16px_50px_rgba(0,0,0,.09)] dark:border-slate-700 dark:bg-slate-800">
            ...
            <p class="mt-1 text-xs font-medium text-green-600 dark:text-green-400">Materi terdaftar</p>
        </div>
        ...
        <div class="rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)] transition duration-300 ease-in-out hover:-translate-y-[3px] hover:shadow-[0_16px_50px_rgba(0,0,0,.09)] dark:border-slate-700 dark:bg-slate-800">
            ...
            <p class="mt-4 text-sm font-medium text-slate-600 dark:text-slate-400">Kategori</p>
            <p class="mt-1 text-3xl font-extrabold text-slate-900 dark:text-slate-100">{{ number_format($stats['kategori']) }}</p>
            <p class="mt-1 text-xs font-medium text-blue-600 dark:text-blue-400">Kategori materi</p>
        </div>
        ...
        <div class="rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)] transition duration-300 ease-in-out hover:-translate-y-[3px] hover:shadow-[0_16px_50px_rgba(0,0,0,.09)] dark:border-slate-700 dark:bg-slate-800">
            ...
            <p class="mt-1 text-xs font-medium text-violet-500 dark:text-violet-400">Soal terkait</p>
        </div>
        ...
        <div class="rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)] transition duration-300 ease-in-out hover:-translate-y-[3px] hover:shadow-[0_16px_50px_rgba(0,0,0,.09)] dark:border-slate-700 dark:bg-slate-800">
            ...
            <p class="mt-4 text-sm font-medium text-slate-600 dark:text-slate-400">Materi Aktif</p>
            <p class="mt-1 text-3xl font-extrabold text-slate-900 dark:text-slate-100">{{ number_format($stats['aktif']) }}</p>
            <p class="mt-1 text-xs font-medium text-orange-500 dark:text-orange-400">{{ $stats['materi'] > 0 ? round($stats['aktif'] / $stats['materi'] * 100) : 0 }}% dari total</p>
        </div>
    </div>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                    @forelse ($materiList as $index => $materi)
                        ...
                        <tr class="transition-colors duration-200 hover:bg-slate-50/60 dark:hover:bg-slate-700/60">
                            <td class="px-4 py-4 text-slate-500 dark:text-slate-400 sm:px-6">{{ $materiList->firstItem() + $index }}</td>
                            <td class="px-4 py-4 sm:px-6">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        <p class="font-semibold text-slate-800 dark:text-slate-100">{{ $materi->judul }}</p>
                                        @if ($deskripsi !== '')
                                            <p class="truncate text-xs text-slate-400 dark:text-slate-500">{{ $deskripsi }}</p>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-4 text-slate-500 dark:text-slate-400 sm:px-6">{{ $materi->urutan }}</td>
                            <td class="px-4 py-4 sm:px-6">
                                @if ($materi->trashed())
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-center text-slate-400 dark:text-slate-500">Tidak ada data materi.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
