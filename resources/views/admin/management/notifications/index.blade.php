<x-admin-layout>
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 sm:text-3xl">Notification Center</h1>
            <p class="mt-1 text-slate-500">Seluruh aktivitas sistem &amp; pemain yang perlu diketahui admin</p>
        </div>
        ...
        <div class="flex shrink-0 items-center gap-2">
            ...
            <button
                ...
                "
                class="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-600 shadow-sm hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60"
            >
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)]">
            ...
            <p class="mt-4 text-sm font-medium text-slate-600">Hari Ini</p>
            <p class="mt-1 text-3xl font-extrabold text-slate-900">{{ number_format($stats['hari_ini']) }}</p>
        </div>
        <div class="rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)]">
            ...
            <p class="mt-4 text-sm font-medium text-slate-600">Minggu Ini</p>
            <p class="mt-1 text-3xl font-extrabold text-slate-900">{{ number_format($stats['minggu_ini']) }}</p>
        </div>
        <div class="rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)]">
            ...
            <p class="mt-4 text-sm font-medium text-slate-600">Bulan Ini</p>
            <p class="mt-1 text-3xl font-extrabold text-slate-900">{{ number_format($stats['bulan_ini']) }}</p>
        </div>
        <div class="rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)]">
            ...
            <p class="mt-4 text-sm font-medium text-slate-600">Belum Dibaca</p>
            <p class="mt-1 text-3xl font-extrabold text-slate-900">{{ number_format($stats['belum_dibaca']) }}</p>
        </div>
    </div>

    {{-- Filters --}}
    <form method="GET" class="mt-6 space-y-4 rounded-3xl border border-slate-100 bg-white p-4 shadow-[0_10px_40px_rgba(0,0,0,.06)] sm:p-5">
        <div class="flex flex-wrap items-center gap-2">
            @foreach (['semua' => 'Semua', 'unread' => 'Belum Dibaca', 'read' => 'Sudah Dibaca'] as $value => $label)
                <a href="{{ route('admin.management.notifications.index', array_merge(request()->except('page'), ['status' => $value])) }}"
                   class="inline-flex items-center gap-2 rounded-xl border px-4 py-2 text-sm font-semibold shadow-sm transition duration-200 ease-in-out {{ $status === $value ? 'border-green-200 bg-green-50 text-green-700' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50' }}">
                    {{ $label }}
                </a>
            @endforeach
        </div>
        ...
        <div class="flex flex-wrap items-center gap-2">
            @foreach ($categoryMeta as $value => $meta)
                <a href="{{ route('admin.management.notifications.index', array_merge(request()->except('page'), ['category' => $value])) }}"
                   class="inline-flex items-center gap-2 rounded-xl border px-3.5 py-2 text-xs font-semibold shadow-sm transition duration-200 ease-in-out {{ $category === $value ? 'border-blue-200 bg-blue-50 text-blue-700' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50' }}">
        ...
        <div class="flex flex-col gap-3 sm:flex-row">
            ...
            <input type="date" name="from" value="{{ $from }}" max="{{ now()->toDateString() }}"
                   class="rounded-xl border-slate-200 text-sm text-slate-700 shadow-sm focus:border-green-500 focus:ring-green-500">
    ...
    <div class="mt-6 space-y-3">
        @forelse ($notifications as $notification)
            ...
            <div class="flex items-start gap-4 rounded-3xl border {{ $notification->read_at ? 'border-slate-100' : 'border-green-200 bg-green-50/30' }} bg-white p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)]">
                ...
                <div class="flex shrink-0 items-center gap-2">
                    @if (!empty($notification->data['url']))
                        <a href="{{ $notification->data['url'] }}" aria-label="Lihat"
                           class="flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 text-slate-500 hover:border-blue-300 hover:bg-blue-50 hover:text-blue-600">
                    ...
                    @unless ($notification->read_at)
                        <form method="POST" action="{{ route('notifications.read', $notification->id) }}">
                            ...
                            <button type="submit" aria-label="Tandai Dibaca"
                                    class="flex h-9 w-9 items-center justify-center rounded-lg border border-green-200 text-green-600 hover:border-green-400 hover:bg-green-50">
                    ...
                    <form method="POST" action="{{ route('notifications.destroy', $notification->id) }}" onsubmit="return confirm('Hapus notifikasi ini?');">
                        ...
                        <button type="submit" aria-label="Hapus"
                                class="flex h-9 w-9 items-center justify-center rounded-lg border border-red-200 text-red-500 hover:border-red-400 hover:bg-red-50">
        ...
        @empty
            <div class="rounded-3xl border border-slate-100 bg-white p-10 text-center text-slate-400 shadow-[0_10px_40px_rgba(0,0,0,.06)]">
