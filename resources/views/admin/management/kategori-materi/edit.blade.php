<x-admin-layout>
    <h1 class="mb-6 text-2xl font-bold text-slate-900 dark:text-slate-100">Edit Kategori Materi</h1>

    <form method="POST" action="{{ route('admin.management.kategori-materi.update', $kategori) }}" class="max-w-xl space-y-4 rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
        @csrf
        @method('PUT')
        ...
        <div class="flex justify-end gap-3 pt-2">
            <a href="{{ route('admin.management.kategori-materi.index') }}" class="rounded-lg px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-700">Batal</a>
