<x-admin-layout>
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 dark:text-slate-100 sm:text-3xl">Kelola Kategori Materi</h1>
            <p class="mt-1 text-slate-500 dark:text-slate-400">Kelola semua kategori materi yang tersedia dalam sistem</p>
        </div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)] transition duration-300 ease-in-out hover:-translate-y-[3px] hover:shadow-[0_16px_50px_rgba(0,0,0,.09)] dark:border-slate-700 dark:bg-slate-800">
            ...
            <p class="mt-4 text-sm font-medium text-slate-600 dark:text-slate-400">Total Kategori</p>
            <p class="mt-1 text-3xl font-extrabold text-slate-900 dark:text-slate-100">{{ number_format($stats['kategori']) }}</p>
            <p class="mt-1 text-xs font-medium text-green-600 dark:text-green-400">Kategori terdaftar</p>
        </div>
        ...
        <div class="rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)] transition duration-300 ease-in-out hover:-translate-y-[3px] hover:shadow-[0_16px_50px_rgba(0,0,0,.09)] dark:border-slate-700 dark:bg-slate-800">
            ...
            <p class="mt-4 text-sm font-medium text-slate-600 dark:text-slate-400">Total Materi</p>
            <p class="mt-1 text-3xl font-extrabold text-slate-900 dark:text-slate-100">{{ number_format($stats['materi']) }}</p>
            <p class="mt-1 text-xs font-medium text-blue-600 dark:text-blue-400">Materi dalam kategori</p>
        </div>
        ...
        <div class="rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)] transition duration-300 ease-in-out hover:-translate-y-[3px] hover:shadow-[0_16px_50px_rgba(0,0,0,.09)] dark:border-slate-700 dark:bg-slate-800">
            ...
            <p class="mt-4 text-sm font-medium text-slate-600 dark:text-slate-400">Total Soal</p>
            <p class="mt-1 text-3xl font-extrabold text-slate-900 dark:text-slate-100">{{ number_format($stats['soal']) }}</p>
            <p class="mt-1 text-xs font-medium text-violet-500 dark:text-violet-400">Soal tersedia</p>
        </div>
        ...
        <div class="rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)] transition duration-300 ease-in-out hover:-translate-y-[3px] hover:shadow-[0_16px_50px_rgba(0,0,0,.09)] dark:border-slate-700 dark:bg-slate-800">
            ...
            <p class="mt-4 text-sm font-medium text-slate-600 dark:text-slate-400">Kategori Aktif</p>
            <p class="mt-1 text-3xl font-extrabold text-slate-900 dark:text-slate-100">{{ number_format($stats['aktif']) }}</p>
            <p class="mt-1 text-xs font-medium text-amber-500 dark:text-amber-400">{{ $stats['kategori'] > 0 ? round($stats['aktif'] / $stats['kategori'] * 100) : 0 }}% dari total</p>
        </div>
    </div>
    ...
    <form method="GET" class="mt-6 rounded-3xl border border-slate-100 bg-white p-4 shadow-[0_10px_40px_rgba(0,0,0,.06)] dark:border-slate-700 dark:bg-slate-800 sm:p-5">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.management.kategori-materi.index', ['filter' => $filter]) }}" aria-label="Reset pencarian"
               class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-green-500 text-green-600 transition duration-200 ease-in-out hover:bg-green-50 dark:hover:bg-green-500/15">
    ...
    <div class="mt-6 overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-[0_10px_40px_rgba(0,0,0,.06)] dark:border-slate-700 dark:bg-slate-800">
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-100 text-sm dark:divide-slate-700">
                <thead class="bg-green-50/60 dark:bg-slate-900/50">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                    @forelse ($kategoriList as $index => $kategori)
                        <tr class="transition-colors duration-200 hover:bg-slate-50/60 dark:hover:bg-slate-700/60">
                            <td class="px-4 py-4 text-slate-500 dark:text-slate-400 sm:px-6">{{ $kategoriList->firstItem() + $index }}</td>
                            <td class="px-4 py-4 sm:px-6">
                                <div class="flex items-center gap-3">
                                    ...
                                    <p class="font-semibold text-slate-800 dark:text-slate-100">{{ $kategori->nama }}</p>
                                </div>
                            </td>
                            <td class="px-4 py-4 text-slate-500 dark:text-slate-400 sm:px-6">{{ $kategori->slug }}</td>
                            <td class="px-4 py-4 text-slate-500 dark:text-slate-400 sm:px-6">{{ $kategori->urutan }}</td>
                            <td class="px-4 py-4 sm:px-6">
                                <span class="inline-flex rounded-full bg-green-100 px-2.5 py-1 text-xs font-semibold text-green-700 dark:bg-green-500/15 dark:text-green-400">
                                    {{ $kategori->materi_count }} Materi
                                </span>
                            </td>
                            <td class="px-4 py-4 sm:px-6">
                                <span class="inline-flex rounded-full bg-blue-100 px-2.5 py-1 text-xs font-semibold text-blue-700 dark:bg-blue-500/15 dark:text-blue-400">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-10 text-center text-slate-400 dark:text-slate-500">Tidak ada data kategori.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
