<x-admin-layout>
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 sm:text-3xl">Kelola Pengguna</h1>
            <p class="mt-1 text-slate-500">Kelola semua akun pengguna pada sistem</p>
        </div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-3xl border border-green-100 bg-green-50/60 p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)] transition duration-300 ease-in-out hover:-translate-y-[3px] hover:shadow-[0_16px_50px_rgba(0,0,0,.09)]">
            ...
            <p class="mt-4 text-sm font-medium text-slate-600">Total Pengguna</p>
            <p class="mt-1 text-3xl font-extrabold text-slate-900">{{ number_format($stats['total']) }}</p>
            <p class="mt-1 text-xs font-medium text-green-600">Akun terdaftar</p>
        </div>

        <div class="rounded-3xl border border-blue-100 bg-blue-50/60 p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)] transition duration-300 ease-in-out hover:-translate-y-[3px] hover:shadow-[0_16px_50px_rgba(0,0,0,.09)]">
            ...
            <p class="mt-4 text-sm font-medium text-slate-600">Pengguna Aktif</p>
            <p class="mt-1 text-3xl font-extrabold text-slate-900">{{ number_format($stats['aktif']) }}</p>
            <p class="mt-1 text-xs font-medium text-blue-600">{{ $stats['total'] > 0 ? round($stats['aktif'] / $stats['total'] * 100) : 0 }}% dari total</p>
        </div>

        <div class="rounded-3xl border border-orange-100 bg-orange-50/60 p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)] transition duration-300 ease-in-out hover:-translate-y-[3px] hover:shadow-[0_16px_50px_rgba(0,0,0,.09)]">
            ...
            <p class="mt-4 text-sm font-medium text-slate-600">Admin</p>
            <p class="mt-1 text-3xl font-extrabold text-slate-900">{{ number_format($stats['admin']) }}</p>
            <p class="mt-1 text-xs font-medium text-orange-500">{{ $stats['total'] > 0 ? round($stats['admin'] / $stats['total'] * 100) : 0 }}% dari total</p>
        </div>

        <div class="rounded-3xl border border-violet-100 bg-violet-50/60 p-5 shadow-[0_10px_40px_rgba(0,0,0,.06)] transition duration-300 ease-in-out hover:-translate-y-[3px] hover:shadow-[0_16px_50px_rgba(0,0,0,.09)]">
            ...
            <p class="mt-4 text-sm font-medium text-slate-600">Pemain</p>
            <p class="mt-1 text-3xl font-extrabold text-slate-900">{{ number_format($stats['pemain']) }}</p>
            <p class="mt-1 text-xs font-medium text-violet-500">{{ $stats['total'] > 0 ? round($stats['pemain'] / $stats['total'] * 100) : 0 }}% dari total</p>
        </div>
    </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($users as $index => $user)
                        ...
                        <tr class="transition-colors duration-200 hover:bg-slate-50/60">
                            <td class="px-4 py-4 text-slate-500 sm:px-6">{{ $users->firstItem() + $index }}</td>
                            ...
                            <td class="px-4 py-4 text-slate-500 sm:px-6">{{ $user->email }}</td>
                            <td class="px-4 py-4 sm:px-6">
                                <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold {{ $isAdmin ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700' }}">
                                    {{ $user->role->label() }}
                                </span>
                            </td>
                            <td class="px-4 py-4 sm:px-6">
                                ...
                                @elseif ($isAdmin && ! $user->hasVerifiedEmail())
                                    <span class="inline-flex items-center gap-1.5 rounded-full bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-600">
                            ...
                            <td class="px-4 py-4 text-slate-500 sm:px-6">
                                <p>{{ $user->created_at->translatedFormat('d M Y') }}</p>
                                <p class="text-xs text-slate-400">{{ $user->created_at->format('H:i') }}</p>
                            </td>
                            <td class="px-4 py-4 sm:px-6">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-10 text-center text-slate-400">Tidak ada data pengguna.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
