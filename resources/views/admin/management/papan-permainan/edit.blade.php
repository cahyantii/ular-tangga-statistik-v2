<x-admin-layout>
    <h1 class="mb-6 text-2xl font-bold text-slate-900 dark:text-slate-100">Edit Papan Permainan</h1>

    <form method="POST" action="{{ route('admin.management.papan-permainan.update', $papan) }}" class="max-w-xl space-y-4 rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
        @csrf
        @method('PUT')
        ...
        <div>
            <label class="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">Jumlah Petak</label>
            <input type="number" value="{{ $papan->jumlah_petak }}" disabled
                   class="w-full rounded-lg border-slate-200 bg-slate-100 text-sm text-slate-400 dark:border-slate-700 dark:bg-slate-900/50 dark:text-slate-500">
            <p class="mt-1 text-xs text-slate-400 dark:text-slate-500">Jumlah petak terkunci setelah papan dibuat.</p>
        </div>
