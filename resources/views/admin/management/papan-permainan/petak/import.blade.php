<x-admin-layout>
    <h1 class="mb-6 text-2xl font-bold text-slate-900">Import Petak — {{ $papan->nama }}</h1>
    ...
    <div class="max-w-2xl space-y-4 rounded-2xl border border-slate-200 bg-white p-6">
        ...
        <code class="block whitespace-pre-wrap rounded-lg bg-slate-50 p-3 text-xs text-slate-700">papan_id, papan_nama, posisi, jenis_petak, kategori, label, icon, warna, border_warna, deskripsi</code>
        <p class="text-sm text-slate-500">
            Hanya baris dengan <strong>papan_id = {{ $papan->id }}</strong> yang diproses. Petak Start/Finish/Tangga/Ular
            tidak bisa diubah lewat import ini — kelola lewat Editor Petak/Konektor. Kolom <strong>kategori</strong>
            diisi nama kategori materi (bukan ID), wajib jika jenis_petak = soal.
        </p>
