<x-admin-layout>
    <h1 class="mb-6 text-2xl font-bold text-slate-900 dark:text-slate-100">Pratinjau Import Petak — {{ $papan->nama }}</h1>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                    @foreach ($invalid as $item)
                        <tr>
                            ...
                            <td class="px-4 py-2 text-slate-700 dark:text-slate-300">{{ $item['raw']['posisi'] ?? '?' }}</td>
            ...
            <table class="min-w-full divide-y divide-slate-100 text-sm dark:divide-slate-700">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                    @foreach ($valid as $item)
                        <tr>
                            <td class="px-4 py-2 text-slate-500 dark:text-slate-400">{{ $item['posisi'] }}</td>
                            <td class="px-4 py-2 text-slate-700 dark:text-slate-300">{{ $item['jenis_petak'] }}</td>
                            <td class="px-4 py-2 text-slate-600 dark:text-slate-400">{{ $item['label'] ?? '-' }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
    ...
    <div class="flex justify-end gap-3">
        <a href="{{ route('admin.management.papan-permainan.petak.import.create', $papan) }}" class="rounded-lg px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-700">Batal</a>
