<x-admin-layout>
    <h1 class="mb-6 text-2xl font-bold text-slate-900 dark:text-slate-100">Edit Petak #{{ $petak->posisi }} — {{ $papan->nama }}</h1>
    ...
    <form method="POST" action="{{ route('admin.management.papan-permainan.petak.update', [$papan, $petak]) }}"
          ...
          class="max-w-xl space-y-4 rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
        @csrf
        @method('PUT')
