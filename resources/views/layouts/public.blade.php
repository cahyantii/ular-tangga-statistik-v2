...
<html lang="id">
    ...
    <body class="overflow-x-hidden bg-white font-sans text-slate-800 antialiased">
        <header x-data="{ mobileMenuOpen: false }" @keydown.escape.window="mobileMenuOpen = false" class="sticky top-0 z-50 bg-white/90 shadow-sm backdrop-blur-md">
            <nav class="relative mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8" aria-label="Navigasi utama">
                ...
                <div class="hidden shrink-0 items-center gap-4 lg:flex">
                    ...
                    @else
                        <a href="{{ route('login') }}" class="text-sm font-semibold text-slate-600 hover:text-primary-600">Masuk</a>
                        <a
                            href="{{ route('register') }}"
                            class="inline-flex shrink-0 items-center whitespace-nowrap rounded-xl bg-gradient-to-r from-primary-500 to-primary-600 px-3.5 py-2 text-xs font-semibold text-white shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:from-primary-600 hover:to-primary-700 hover:shadow-soft sm:px-5 sm:py-2.5 sm:text-sm"
                        >
                ...
                <button
                    ...
                    class="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-lg text-slate-700 transition-colors duration-200 hover:bg-slate-100 lg:hidden"
            ...
            <div
                ...
                id="mobile-menu"
                class="absolute inset-x-0 top-full origin-top rounded-b-2xl border-b border-slate-100 bg-white shadow-lg lg:hidden"
            >
                <div class="mx-auto max-w-7xl px-4 py-4 sm:px-6">
                    <div class="flex flex-col gap-1 text-sm font-semibold text-slate-600">
                        @foreach ([
                            ...
                            <a
                                href="{{ route($item['route']) }}"
                                aria-current="{{ $active ? 'page' : 'false' }}"
                                class="rounded-lg border-b-2 px-3 py-2.5 transition-colors duration-200 {{ $active ? 'border-primary-600 font-semibold text-primary-600' : 'border-transparent hover:bg-slate-50 hover:text-primary-600' }}"
                            >
                                {{ $item['label'] }}
                            </a>
                        @endforeach
                    ...
                    <div class="flex flex-col gap-3">
                        ...
                        @else
                            <a
                                href="{{ route('login') }}"
                                class="inline-flex w-full items-center justify-center rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-semibold text-slate-600 transition-colors duration-200 hover:border-primary-200 hover:text-primary-600"
                            >
                                Masuk
                            </a>
                            <a
