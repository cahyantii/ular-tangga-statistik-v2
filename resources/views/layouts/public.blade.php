...
<html lang="id">
    <head>
        ...
        {{-- Anti-FOUC: terapkan tema dari preferensi player --}}
        <script>
            (function () {
    ...
    <body class="overflow-x-hidden bg-white font-sans text-slate-800 antialiased dark:bg-slate-900 dark:text-slate-200">
        <header x-data="{ mobileMenuOpen: false }" @keydown.escape.window="mobileMenuOpen = false" class="sticky top-0 z-50 bg-white/90 shadow-sm backdrop-blur-md dark:bg-slate-900/90 dark:shadow-slate-800/50">
            <nav class="relative mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8" aria-label="Navigasi utama">
                ...
                <div class="hidden items-center gap-8 text-sm font-semibold text-slate-600 lg:flex dark:text-slate-300">
                    @foreach ([
                        ...
                        <a
                            ...
                            class="border-b-2 pb-1 transition-colors duration-200 {{ $active ? 'border-primary-500 text-primary-500' : 'border-transparent hover:text-primary-500 dark:hover:text-primary-400' }}"
                        >
                            {{ $item['label'] }}
                        </a>
                    @endforeach
                ...
                <button
                    ...
                    class="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-lg text-slate-700 transition-colors duration-200 hover:bg-slate-100 lg:hidden dark:text-slate-300 dark:hover:bg-slate-800"
            ...
            <div
                ...
                id="mobile-menu"
                class="absolute inset-x-0 top-full origin-top rounded-b-2xl border-b border-slate-100 bg-white shadow-lg lg:hidden dark:border-slate-700 dark:bg-slate-900"
            >
                    ...
                    <div class="flex flex-col gap-1 text-sm font-semibold text-slate-600 dark:text-slate-300">
                        @foreach ([
                            ...
                            <a
                                href="{{ route($item['route']) }}"
                                aria-current="{{ $active ? 'page' : 'false' }}"
                                class="rounded-lg border-b-2 px-3 py-2.5 transition-colors duration-200 {{ $active ? 'border-primary-500 font-semibold text-primary-500' : 'border-transparent hover:bg-slate-50 hover:text-primary-600 dark:hover:bg-slate-800 dark:hover:text-primary-400' }}"
                            >
                                {{ $item['label'] }}
                            </a>
                        @endforeach
        ...
        <footer class="border-t border-slate-100 dark:border-slate-700/60">
            <div class="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-4 py-10 text-sm text-slate-500 sm:flex-row sm:px-6 lg:px-8 dark:text-slate-400">
