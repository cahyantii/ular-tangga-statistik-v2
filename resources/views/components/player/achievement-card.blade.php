...
@php
    ...
    $palette = [
        'blue' => ['bg' => 'bg-primary-50 dark:bg-primary-500/10', 'border' => 'border-primary-200 dark:border-primary-500/30', 'icon' => 'bg-primary-500', 'bar' => 'bg-primary-500', 'text' => 'text-primary-600 dark:text-primary-400'],
        'green' => ['bg' => 'bg-secondary-50 dark:bg-secondary-500/10', 'border' => 'border-secondary-200 dark:border-secondary-500/30', 'icon' => 'bg-secondary-500', 'bar' => 'bg-secondary-500', 'text' => 'text-secondary-600 dark:text-secondary-400'],
        'amber' => ['bg' => 'bg-accent-50 dark:bg-accent-500/10', 'border' => 'border-accent-200 dark:border-accent-500/30', 'icon' => 'bg-accent-500', 'bar' => 'bg-accent-500', 'text' => 'text-accent-600 dark:text-accent-400'],
        'purple' => ['bg' => 'bg-violet-50 dark:bg-violet-500/10', 'border' => 'border-violet-200 dark:border-violet-500/30', 'icon' => 'bg-violet-500', 'bar' => 'bg-violet-500', 'text' => 'text-violet-600 dark:text-violet-400'],
        'rose' => ['bg' => 'bg-rose-50 dark:bg-rose-500/10', 'border' => 'border-rose-200 dark:border-rose-500/30', 'icon' => 'bg-rose-500', 'bar' => 'bg-rose-500', 'text' => 'text-rose-600 dark:text-rose-400'],
    ];
...
<div
    class="group relative overflow-hidden rounded-2xl border p-5 transition-all duration-200 hover:-translate-y-1 hover:shadow-soft
        {{ $earned ? "{$tone['bg']} {$tone['border']}" : 'border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800' }}"
>
        ...
        @else
            <span
                class="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl shadow-sm transition-transform duration-200 group-hover:scale-105
                    {{ $isLocked ? 'bg-slate-200 text-slate-400 dark:bg-slate-700 dark:text-slate-500' : "{$tone['icon']} text-white" }}"
            >
    ...
    @else
        <div class="mt-4 flex items-center justify-between">
            ...
            @else
                <span class="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-500 dark:bg-slate-700 dark:text-slate-400">
                    <x-player.icon name="lock" class="h-3 w-3" />
                    Terkunci
                </span>
            @endif
