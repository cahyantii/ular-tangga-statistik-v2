...
@php
    $palette = [
        ...
        'green' => 'bg-secondary-50 text-secondary-600 dark:bg-secondary-500/10 dark:text-secondary-400',
        'amber' => 'bg-accent-50 text-accent-600 dark:bg-accent-500/10 dark:text-accent-400',
        'purple' => 'bg-violet-50 text-violet-600 dark:bg-violet-500/10 dark:text-violet-400',
        'rose' => 'bg-rose-50 text-rose-600 dark:bg-rose-500/10 dark:text-rose-400',
    ];
@endphp

<div class="animate-fade-in-up rounded-3xl bg-white p-5 shadow-sm transition-shadow duration-300 hover:shadow-soft dark:bg-slate-800 dark:shadow-none sm:p-6">
    ...
    @if ($activities->isEmpty())
        <div class="flex flex-col items-center gap-3 py-8 text-center sm:flex-row sm:gap-5 sm:py-6 sm:text-left">
            <span class="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-slate-100 text-slate-300 dark:bg-slate-700 dark:text-slate-600">
            ...
            <div>
                ...
                <p class="mt-1 text-sm text-slate-400 dark:text-slate-500">Mulai mengerjakan soal untuk melihat riwayat aktivitasmu di sini.</p>
            </div>
        </div>
    @else
            ...
            @foreach ($activities as $activity)
                <li class="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="text-xs text-slate-400 dark:text-slate-500">{{ $activity['subtitle'] }}</p>
                    </div>
                    <span class="shrink-0 text-xs text-slate-400 dark:text-slate-500">{{ $activity['at']->diffForHumans() }}</span>
                </li>
            @endforeach
        </ul>
    @endif
