...
{{--
    premium = true  → dipakai di halaman Sertifikat (cert-card--cream via CSS)
    premium = false → card amber biasa dengan Tailwind classes
--}}
<div @class([
    'relative animate-fade-in-up p-5 sm:p-6',
    'cert-card cert-card--cream lg:p-7' => $premium,
    'rounded-3xl bg-accent-50 shadow-sm transition-shadow duration-300 hover:shadow-soft dark:bg-amber-950/40 dark:border dark:border-amber-900/40' => ! $premium,
])>
    @if ($illustration && $illustrationPosition === 'right')
        <div class="pr-[110px] sm:pr-[120px] lg:pr-[130px]">
            <h3 class="tips-title flex items-center gap-2 text-base font-bold text-slate-700 dark:text-slate-200">
            ...
            <p class="mt-3 text-sm leading-relaxed text-slate-600 dark:text-slate-300">{{ $tip }}</p>
        </div>
