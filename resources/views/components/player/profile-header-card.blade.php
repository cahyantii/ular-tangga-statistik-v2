...
<div class="relative overflow-hidden rounded-3xl bg-white p-6 shadow-sm sm:p-8 dark:bg-dark-surface">
    <div
    ...
    <div
        class="pointer-events-none absolute inset-0 block dark:hidden sm:hidden"
        style="background: linear-gradient(180deg, rgba(255,255,255,0.94) 0%, rgba(255,255,255,0.9) 55%, rgba(255,255,255,0.8) 100%);"
        aria-hidden="true"
    ></div>
    <div
        class="pointer-events-none absolute inset-0 hidden dark:max-sm:block"
        style="background: linear-gradient(180deg, rgba(15,23,42,0.94) 0%, rgba(15,23,42,0.9) 55%, rgba(15,23,42,0.8) 100%);"
        aria-hidden="true"
    ></div>

    {{-- Tablet & desktop: konten rata kiri, jadi overlay memudar dari kiri ke kanan --}}
    <div
        class="pointer-events-none absolute inset-0 hidden sm:block dark:sm:hidden"
    ...
    ></div>
    <div
        class="pointer-events-none absolute inset-0 hidden dark:sm:block"
        style="background: linear-gradient(90deg, rgba(15,23,42,0.92) 0%, rgba(15,23,42,0.78) 35%, rgba(15,23,42,0.35) 70%, rgba(15,23,42,0.10) 100%);"
        aria-hidden="true"
    ></div>
    ...
    <div class="relative z-10 flex flex-col items-center gap-5 sm:flex-row sm:items-center">
        <div class="relative shrink-0">
            ...
            <div class="relative h-[110px] w-[110px] rounded-full bg-blue-50 p-1.5 shadow-md ring-1 ring-primary-400/20 transition-shadow duration-300 hover:shadow-lg md:h-[130px] md:w-[130px] lg:h-[140px] lg:w-[140px]">
                <div class="h-full w-full overflow-hidden rounded-full border-4 border-white dark:border-dark-surface">
            ...
            <a
                ...
                class="absolute -bottom-1 -right-1 flex h-8 w-8 items-center justify-center rounded-full border-2 border-white dark:border-dark-surface bg-primary-500 text-white shadow-sm transition hover:bg-primary-600"
            >
        ...
        <div class="min-w-0 text-center sm:text-left">
            <div class="flex flex-wrap items-center justify-center gap-2 sm:justify-start">
                <h2 class="text-xl font-bold text-slate-800 dark:text-dark-text">{{ $user->name }}</h2>
                <x-player.badge :label="$summary['level_name']" :color="$summary['badge_color']" />
            </div>

            <p class="mt-1 text-sm text-slate-500 dark:text-dark-muted">Pemain sejak {{ $user->created_at->translatedFormat('d F Y') }}</p>

            <div class="mt-2.5 flex flex-wrap items-center justify-center gap-4 text-sm sm:justify-start">
                <span class="flex items-center gap-1.5 font-semibold text-accent-600 dark:text-accent-400">
