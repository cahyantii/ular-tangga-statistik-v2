...
{{-- Use .leaderboard-row-highlight CSS class for gradient that works in dark mode --}}
<tr class="transition-colors duration-200 {{ $highlight ? 'leaderboard-row-highlight' : 'hover:bg-slate-50 dark:hover:bg-slate-700/50' }}">
    <td class="px-4 py-3.5 sm:px-6">
        ...
        @else
            <span class="font-semibold text-slate-500 dark:text-slate-400">{{ $row['rank'] }}</span>
        @endif
    </td>
    ...
    <td class="px-4 py-3.5 sm:px-6">
        <div class="flex min-w-0 items-center gap-3">
            ...
            <span class="truncate font-semibold text-slate-800 dark:text-slate-100">{{ $row['nama'] }}</span>
        </div>
    </td>
    ...
    <td class="px-4 py-3.5 text-right sm:px-6">
        <span class="inline-flex items-center gap-1.5 font-bold text-accent-600 dark:text-amber-400">
