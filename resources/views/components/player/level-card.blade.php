...
<div class="animate-fade-in-up rounded-3xl bg-white p-5 shadow-sm transition-shadow duration-300 hover:shadow-soft sm:p-6">
    ...
    <div class="mt-4 flex items-center gap-4">
        <div class="relative flex h-16 w-16 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-accent-400 to-accent-600 text-white shadow-sm">
            ...
            <span class="absolute -bottom-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full border-2 border-white bg-primary-500 text-xs font-bold text-white">
                {{ $level['level'] }}
            </span>
        </div>

        <div class="min-w-0">
            <p class="font-bold text-slate-800">{{ $level['level_name'] }}</p>
            <p class="text-xs text-slate-400">{{ $level['xp_into_level'] }} / {{ $level['xp_for_next_level'] }} XP</p>
        </div>
    </div>
